...
def cmd_optimize(args) -> int:
    ...
    print(f"always-loaded tokens: {result['before_always_loaded_tokens']} -> {result['after_always_loaded_tokens']} "
    ...
    print("see the duplicate_groups / likely_unnecessary sections of `weeder audit`, and SKILL.md's workflow.")
    return 0
...
def cmd_apply_suggestion(args) -> int:
    ...
    print(f"applied: {', '.join(result['applied']) or '(nothing -- answer file had no recognized keys)'}")
    for w in result["warnings"]:
        print(f"  warning: {w}")
    print("\nThis only applies the answer mechanically; it doesn't validate it -- run")
    print("`weeder test-routing`/`test-function`/`diff` against the output copy before accepting it")
...
def cmd_test_routing(args) -> int:
    ...
    print(f"routing accuracy: {result['accuracy']:.0%}  ({result['n']} example(s))" if result["accuracy"] is not None
          else "no examples given")
    for r in result["results"]:
        mark = "OK  " if r["correct"] else "MISS"
        print(f"  [{mark}] {r['kind']:<10} expected={r['expected']:<10} predicted={r['predicted']!r:<20} \"{r['prompt'][:60]}\"")
...
def cmd_test_function(args) -> int:
    ...
    if args.json:
        ...
        print(f"preserved in after (found somewhere in SKILL.md+references): {result['preserved']} "
...
def cmd_diff(args) -> int:
    ...
    if before_report["always_loaded_tokens"]:
        reduction = 100 * (1 - after_report["always_loaded_tokens"] / before_report["always_loaded_tokens"])
    ...
    if args.examples:
        ...
        r_before = evaluate_routing(before_parsed["name"], before_parsed["frontmatter"].get("description", ""), examples, competing)
        r_after = evaluate_routing(after_parsed["name"], after_parsed["frontmatter"].get("description", ""), examples, competing)
        print()
        print("Routing (lexical-overlap proxy, not a real-model prediction -- see references/routing-heuristic.md):")
        print(f"  Before {r_before['accuracy']:.0%}" if r_before["accuracy"] is not None else "  Before n/a")
        print(f"  After  {r_after['accuracy']:.0%}" if r_after["accuracy"] is not None else "  After  n/a")

    func = check_constraints_preserved(parse_skill(args.before), parse_skill(args.after))
    print()
    print("Functional (constraint-preservation proxy, not a behavioral eval -- see validate.md):")
    print(f"  Constraints preserved: {func['preserved']}/{func['total']} ({func['preserved_ratio']:.0%})")
    if func["missing"]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_audit = sub.add_parser("audit", help="structural classification + token accounting + duplication + unnecessary-content flags")
    p_audit.add_argument("skill_dir")
    p_audit.add_argument("--dup-threshold", type=float, default=0.6)
    p_audit.add_argument("--json", action="store_true")

    p_opt = sub.add_parser("optimize", help="mechanically move background/examples sections to references/")
    ...
    p_suggest = sub.add_parser("suggest", help="structure step 3's judgment request for the invoking agent, gated by --assist level")
    p_suggest.add_argument("skill_dir")
    p_suggest.add_argument("--assist", choices=LEVELS, default=None,
    ...
    p_apply = sub.add_parser("apply-suggestion", help="mechanically apply an agent-authored answer (see `suggest`) to a copy of the skill")
    p_apply.add_argument("skill_dir")
    p_apply.add_argument("--answer", required=True, help="JSON file answering one or more of suggest's requests")
    p_apply.add_argument("--out", default=None, help="output dir (default: <skill_dir>-optimized)")
    p_apply.add_argument("--dup-threshold", type=float, default=0.6,
    ...
    p_route = sub.add_parser("test-routing", help="evaluate routing accuracy against labeled example prompts")
    p_route.add_argument("skill_dir")
    p_route.add_argument("--examples", required=True, help="JSON file: {positive:[], negative:[], ambiguous:[{prompt,expected}]}")
    p_route.add_argument("--competing", nargs="*", default=None, help="other skill dirs to include as routing competitors")
    p_route.add_argument("--min-accuracy", type=float, default=0.0)
    p_route.add_argument("--json", action="store_true")

    p_func = sub.add_parser("test-function", help="check before-skill's constraints are still present somewhere in after-skill")
    p_func.add_argument("before")
    p_func.add_argument("after")
    p_func.add_argument("--threshold", type=float, default=0.6, help="keyword-coverage ratio to count a constraint as preserved")
    p_func.add_argument("--min-ratio", type=float, default=1.0, help="exit nonzero if preserved_ratio falls below this")
...
def main(argv=None) -> int:
    ...
    handlers = {
        "audit": cmd_audit, "optimize": cmd_optimize, "suggest": cmd_suggest,
        "apply-suggestion": cmd_apply_suggestion, "test-routing": cmd_test_routing,
        "test-function": cmd_test_function, "diff": cmd_diff,
    }
    try:
