...
def residual_norm(
    A, x, b, tol: float, ord: int | str = 2, name: str = "linear_system"
) -> CheckResult:
    ...
    return CheckResult(
        ...
        status=status.value,
        category="numerical",
        metric={"residual_norm": norm, "ord": str(ord)},
        expected=f"<= {tol}",
        observed=norm,
...
def conditioning(
    A, warn_threshold: float = 1e8, fail_threshold: float = 1e12, name: str = "matrix"
) -> CheckResult:
    A = np.asarray(A, dtype=float)
...
def reconstruction_error(
    original, reconstructed, tol: float, name: str = "reconstruction"
) -> CheckResult:
