...
def timestep_convergence(solve_fn, dts, reference=None, expected_order: float | None = None,
                          tol_order: float = 0.3, error_fn=None, name: str = "timestep_convergence") -> CheckResult:
    ...
    return convergence_check(solve_fn, dts, reference=reference, expected_order=expected_order,
                              tol_order=tol_order, error_fn=error_fn, name=name, param_label="dt")


def invariant_preservation(trajectory, invariant_fn, tol: float, name: str = "invariant") -> CheckResult:
    ...
    if values.size == 0:
        return CheckResult(
            name=f"invariant_preservation:{name}", status=Status.WARN.value, category="numerical",
            metric={}, expected=f"<= {tol}", observed=None, evidence={}, notes="Empty trajectory.",
        )
    ...
    return CheckResult(
        name=f"invariant_preservation:{name}", status=status.value, category="numerical",
...
def reference_solution_comparison(numerical, analytic, tol: float, name: str = "reference_solution") -> CheckResult:
    ...
    return CheckResult(
        name=f"reference_solution:{name}", status=status.value, category="model_validation",
